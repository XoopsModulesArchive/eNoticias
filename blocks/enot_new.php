...
//  ------------------------------------------------------------------------ //
function b_enot_new_show($options)
{
        ...
        if ($i < $options[1]) {
            ...
            if ($noticiaModule = $hModule->getByDirname('eNoticias')) {
                ...
                if ($gpermHandler->checkRight('Column Permissions', $myrow['columnID'], $groups, $module_id)) {
                    ...
                    $newarts['linktext'] = $linktext;

                    $newarts['id'] = $myrow['articleID'];

                    $newarts['dir'] = $module_name;

                    if ('datesub' == $options[0]) {
                        $newarts['new'] = formatTimestamp($myrow['datesub'], $noticiaConfig['dateformat']);
                    } elseif ('counter' == $options[0]) {
                        $newarts['new'] = $myrow['counter'];
                    } elseif ('weight' == $options[0]) {
                        $newarts['new'] = $myrow['weight'];
                    } elseif ('rating' == $options[0]) {
                        $newarts['new'] = number_format($myrow['rating'], 2, '.', '');

                        $newarts['votes'] = $myrow['votes'];
                    }

                    $i++;

                    $block['newarts'][] = $newarts;
                }
            }
        }
    }
...
function b_enot_new_edit($options)
{
